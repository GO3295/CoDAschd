...
#'
#' This function transforms the raw scRNA-seq matrix to different CoDA log-ratio-based matrix. To avoid zeros, different count addition could be used.
#' @param dat The raw count scRNA-seq matrix (gene(row) x cell(column)).
#' @param countAdd The 'count' added to the whole matrix to avoid zero. Options included: s/gm (sum/geometric mean(sum))(default); s/max (sum/maximum(sum)); s/10000; user-defined number(e.g.,1,0.1). s refers to the total count of each cell.
...
#' HKGLR_dat <- runCoDAschd_CountAdd(raw_dat,countAdd="s/10000",method="manual",gene=c("GAPDH","UBC"));
#' @export
